...
public class Addition extends UnicastRemoteObject implements AdditionInterface{
	...
	public String hesapEkle(int NO, String NM) throws RemoteException{
		try {
			...
			//burada hesap listesini kayýt yaptým
			hesapInsert.add(set);
			hesapList.add(set);
	...
	public String bakiyeYatýrým(int NO, int TL) throws RemoteException{
		//ArrayList<HesapList> hesapList = new ArrayList<HesapList>();
		try {
			//arraylistteki hesaptaki para bilgilerini alýp üzerine yeni para tutarýný ekleyerek tekrar arrayliste gönderir
	...
	public String bakiyeCek(int NO, int TL) throws RemoteException{
		//ArrayList<HesapList> hesapList = new ArrayList<HesapList>();
		try {
			//Arraylist içindeki verileri Iterator sayesinde okuyorum
			Iterator itr = hesapList.iterator();
			int index = 0;
			System.out.println("Para çekme");
			//While ile ile Iterator kadar döndürüp gelen hesap numarasýna göre veriyi güncelliyorum
			 while(itr.hasNext()){  
		        	HesapList st = (HesapList)itr.next();  
		        	int num = st.no;
		            ...
		            if (num == NO) {
		            	if (ACCUUNTMONEY >= TL) {
		            		//Veri güncelleme ve para çekme gelen deðer ile hesaptaki bakiyeyi çýkarýyorum.
	...
	public String hesapListe() throws RemoteException{
		try {
			//hesaplist arrayindeki bilgileri erkana bastýrýr
			System.out.println("");
			Iterator itr = hesapList.iterator();  
			 
			//while döngüsünde hesaplistteki bilgileri satýr satýr düzenleri olarak getirir
	        while(itr.hasNext()){  
	        	HesapList st = (HesapList)itr.next();
	        	System.out.println("Hesap No : " + st.no + " Hesap Taným : " 
	...
	public String musteriEkle(int NO, String NM, int ACCUUNTNO) throws RemoteException{
		try {
			...
			Iterator itr = hesapList.iterator();
			int index = 0;
			//customerInsert.add(set); bu komut hesapliste müþteri ekliyor
			 while(itr.hasNext()){  
		        	HesapList st = (HesapList)itr.next();  
		        	int num = st.no;
		            if (num == ACCUUNTNO) {
	...
	public String musteriListe() throws RemoteException{
		try {
			//burada müþterileri listeliyor
			Iterator itr = customerInsert.iterator();  
			 
	        while(itr.hasNext()){  
	        	CUSTOMER st = (CUSTOMER)itr.next();
	            System.out.println("Müþteri No : " + st.id + ", Müþteri Adý : " +
	        	st.musteriAd + ", Müþterinin Hesabý : " + st.musteriHesapNo);  
	        }
